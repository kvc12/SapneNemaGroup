...
export default class LoginVendor extends Component {
    ...
    handleSubmit = async (event) => {
        ...
        service.loginVendor(this.state.loginvendor.vendorUsername, this.state.loginvendor.vendorPassword)
            .then((result) => {
                if (result.data != null) {
                    sessionStorage.setItem("username", this.state.loginvendor.vendorUsername)
                    sessionStorage.setItem("password", this.state.loginvendor.vendorPassword)
                            ...
                            <form onSubmit={this.handleSubmit}>
                                ...
                                <div className="mt-5">
                                    <TextField id="username" label="Enter Username" data-testid="userName" variant="outlined" fullWidth
                                        ...
                                        value={this.state.loginvendor.vendorUsername}
                                        onChange={
                                            (e) => {
                                                this.setState({ loginvendor: { ...this.state.loginvendor, vendorUsername: e.target.value } })
                                            }
                                        }
                                    />
                                ...
                                <div className="mt-5">
                                    <TextField id="password" label="Enter Password" variant="outlined" data-testid="password" fullWidth
                                        InputProps={{
                                            startAdornment: (
                                                <InputAdornment position="start">
                                                    <LockIcon />
                                        ...
                                        value={this.state.loginvendor.vendorPassword}
                                        onChange={(e) =>
